...
import { GenericFuzzySuggester, SuggesterItem } from '../ui/GenericFuzzySuggester2';
  ...
  try {
    ...
    if (await plugin.app.vault.adapter.exists(filePath)) {
      ...
      if (command === 'BOTTOM' || command !== 'TOP') {
        if (command === 'BOTTOM') lineNumber = fileBkmrkContentsArray.length - 1;
        else {
...
export async function createTagFileListChooser(
  ...
  await tagChooser.display(async (tagChosen: SuggesterItem) => {
    ...
    for (const tag of filesForChosenTag) tagFileListArray.push({ display: tag, info: tag });
...
export async function createTagBlockListChooser(
  ...
  await tagChooser.display(async (tagChosen: SuggesterItem) => {
    ...
    for (const tag of await tagBlocks) tagFileListArray.push({ display: tag.file + '\n' + tag.blockText, info: tag });
...
export async function displayFileLineSuggesterFromFileList(
  ...
  if (fileContentsStartingLine > 0) firstLinechooser.setSuggesterData(lineCountArray.slice(fileContentsStartingLine));
