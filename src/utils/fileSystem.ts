import { type App, type TAbstractFile, TFolder, Vault } from "obsidian";
...
function testFolderExclusion(
	folder: string,
	exclusionFolders: Array<string>,
): boolean {
	// return  true if should be excluded
	for (const eFolder of exclusionFolders)
		if (folder.startsWith(`${eFolder}/`)) return true;
	return false;
}

async function getFiles(
	app: App,
	returnType: FileSystemReturnType,
	responseArray: Array<SuggesterItem>,
	exclusionFolders: Array<string>,
) {
	// first list just files
	if (
		returnType === FileSystemReturnType.filesOnly ||
		returnType === FileSystemReturnType.filesAndFolders
	)
	...
	// second list folders
	if (
		returnType === FileSystemReturnType.foldersOnly ||
		returnType === FileSystemReturnType.filesAndFolders
	) {
...
async function addLastOpenFiles(app: App, responseArray: Array<SuggesterItem>) {
	...
	for (let i = lastOpenFiles.length - 1; i >= 0; i--)
		responseArray.unshift({
			display: `Recent file: ${lastOpenFiles[i]}`,
			info: lastOpenFiles[i],
		}); //add file to array
}
...
export default class FileSystem {
	...
	async getAllFolders(): Promise<Array<SuggesterItem>> {
		...
		await getFiles(
			this.plugin.app,
			FileSystemReturnType.foldersOnly,
			results,
	...
	async getAllFiles(): Promise<Array<SuggesterItem>> {
		...
		await getFiles(
			this.plugin.app,
			FileSystemReturnType.filesOnly,
			results,
			this.exclusionFolders,
		);
	...
	async getAllFoldersAndFiles(): Promise<Array<SuggesterItem>> {
		...
		await getFiles(
			this.plugin.app,
			FileSystemReturnType.filesAndFolders,
			results,
			this.exclusionFolders,
		);
		await addLastOpenFiles(this.plugin.app, results);
