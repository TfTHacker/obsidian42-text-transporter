...
export async function addBookmarkFromCurrentView(plugin: TextTransporterPlugin): Promise<void> {
  ...
  locationChooser.display((location: SuggesterItem) => {
    ...
    if (location) {
      ...
      if (plugin.settings.bookmarks.split('\n').find((b) => b === newBookmark)) new Notice(`The bookmark: ${newBookmark} already exists.`);
      else {
...
export async function openBookmark(plugin: TextTransporterPlugin): Promise<void> {
  ...
  if (bookmarks.length === 0) new Notice('There are no bookmarks defined.');
    ...
    for (let i = bookmarks.length - 1; i >= 0; i--) fileList.unshift({ display: bookmarks[i], info: bookmarks[i] });
