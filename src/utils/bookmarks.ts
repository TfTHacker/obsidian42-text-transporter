...
export async function addBookmarkFromCurrentView(
	...
	const currentLineText = currentView.editor.getLine(
		currentView.editor.getCursor().line,
	);
	...
	data.push({
		display:
			"TOP: Bookmark the top of the file and mark as a context menu location",
		info: "TOP*",
	});
	data.push({
		display: "BOTTOM: Bookmark the bottom of the file ",
		info: "BOTTOM",
	});
	data.push({
		display:
			"BOTTOM: Bookmark the bottom of the file and mark as a context menu location",
		info: "BOTTOM*",
	});
	if (currentLineText.length > 0) {
		data.push({
			display: `Location: of selected text "${currentLineText}"`,
...
export async function removeBookmark(
	...
	if (bookmarks.length === 0) new Notice("There are no bookmarks defined.");
		...
		const data = new Array<SuggesterItem>();
		for (const b of bookmarks) data.push({ display: b, info: b });
