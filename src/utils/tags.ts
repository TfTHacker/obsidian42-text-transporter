import type { App, CachedMetadata, Pos } from "obsidian";
...
export function getAllTagsJustTagNames(): string[] {
	//@ts-ignore
	return Object.keys(app.metadataCache.getTags()).sort((a, b) =>
		a.localeCompare(b),
	);
}
...
export function locationsWhereTagIsUsed(findTag: string): Array<TagLocation> {
	...
	return results.sort((a: TagLocation, b: TagLocation) =>
...
export async function blocksWhereTagIsUsed(
	...
	for (const l of locationsWhereTagIsUsed(findTag)) {
		...
		if (block.type !== "yaml") {
			...
			let blockText = "";
			for (const line of taggedFileArray.slice(
				block.lineStart,
				block.lineEnd + 1,
			))
