...
export function locationsWhereTagIsUsed(findTag: string): Array<TagLocation> {
  ...
  for (const file of oApp.vault.getMarkdownFiles()) {
    ...
    if (cache.tags)
      for (const tag of cache.tags) if (findTag === tag.tag) results.push({ tag: tag, filePath: file.path, position: tag.position });
  }
...
export function filesWhereTagIsUsed(findTag: string): string[] {
  const filesList = [];
  for (const l of locationsWhereTagIsUsed(findTag)) if (!filesList.includes(l.filePath)) filesList.push(l.filePath);
  return filesList;
}
...
export async function blocksWhereTagIsUsed(plugin: TextTransporterPlugin, findTag: string): Promise<string[]> {
  ...
  for (const l of locationsWhereTagIsUsed(findTag)) {
    ...
    if (block.type !== 'yaml') {
      ...
      let blockText = '';
      for (const line of taggedFileArray.slice(block.lineStart, block.lineEnd + 1)) blockText += line.display + '\n';
