...
export class FileCacheAnalyzer {
	...
	constructor(plugin: TextTransporterPlugin, fileFullPath: string) {
		this.plugin = plugin;
		this.cache = <CachedMetadata>(
			plugin.app.metadataCache.getCache(fileFullPath)
		);
	...
	getBlockAfterLine(line: number): CacheDetails {
		...
		if (this.details.length - 1 > blockIndexAtLine)
	...
	getBlockBeforeLine(line: number): CacheDetails {
		...
		if (blockNumberAtLine > 0 && this.details.length >= blockNumberAtLine)
	...
	breakdownCacheItems(
		cacheItems: Array<CacheItem>,
		section: SectionCache,
		checkForBlockRefs: boolean,
	): void {
		let itemsFoundTrackToBreakOut = false;
		for (const itemInCache of cacheItems) {
			const positionInSameRange = this.positionOfItemWithinSameRange(
			...
			if (positionInSameRange) {
				...
				if (checkForBlockRefs && this.cache.blocks) {
					...
					for (const b of Object.values(this.cache.blocks)) {
						if (
							this.positionOfItemWithinSameRange(
								b.position,
								itemInCache.position,
							)
						) {
	...
	positionOfItemWithinSameRange(
		firstPosition: Pos,
		secondPosition: Pos,
	): boolean {
		return (
			firstPosition.start.line >= secondPosition.start.line &&
			firstPosition.end.line <= secondPosition.end.line
		);
	}
}
