...
export function selectAdjacentBlock(plugin: TextTransporterPlugin, nextBlock: boolean): void {
  const activeView = getActiveView(plugin);
  const activeEditor = activeView.editor;
  const currentLine = activeEditor.getCursor().line;
...
export function indentifyCurrentSection(plugin: TextTransporterPlugin): SectionCache {
  const activeView = getActiveView(plugin);
  const activeEditor = activeView.editor;
  const currentLine = activeEditor.getCursor().line;
  const cache = this.app.metadataCache.getFileCache(activeView.file);
  return cache.sections.find((section) => section.position.start.line <= currentLine && section.position.end.line >= currentLine);
}

// Select the current section in the editor of activeLeaf and extend the selection in a given direction
export function selectCurrentSection(plugin: TextTransporterPlugin, directionUP = true): void {
  const activeView = getActiveView(plugin);
  const activeEditor = activeView.editor;
  const currentLine = activeEditor.getCursor().line;
  const cache = this.app.metadataCache.getFileCache(activeView.file);
  ...
  if (
    (currentRange[0].anchor.line === currentRange[0].head.line && currentRange[0].head.ch !== activeEditor.getSelection().length) ||
    (currentRange[0].head.ch === 0 &&
      currentRange[0].anchor.ch === 0 &&
      activeEditor.getRange({ line: currentLine, ch: activeEditor.getLine(currentLine).length }, { line: currentLine, ch: 0 }).length !== 0)
  ) {
    ...
    if (lastLineOfBlock === undefined) {
      ...
      if (nearestBlock === null && currentLine === 0 && f.details.length > 0) nearestBlock = cache.sections[0]; // first line, but no text to select, so select first  block
    ...
    if (
      ...
      for (let i = 0; i < f.details.length; i++) {
        ...
        if (firstSelectedLine > f.details[i].position.end.line) proceedingBlock = f.details[i];
      }
