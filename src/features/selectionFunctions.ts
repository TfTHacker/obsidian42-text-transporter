...
export function indentifyCurrentSection(plugin: ThePlugin): SectionCache {
    const activeView = getActiveView(plugin);
    const activeEditor = activeView.editor;  
    const currentLine = activeEditor.getCursor().line;
...
export function selectCurrentSection(plugin: ThePlugin, directionUP = true): void {
    ...
    if (currentRange[0].anchor.line === currentRange[0].head.line &&
        (currentRange[0].head.ch !== activeEditor.getSelection().length) || (currentRange[0].head.ch === 0 && currentRange[0].anchor.ch === 0) &&
        ...
        if (lastLineOfBlock && lastLineOfBlock.type === "paragraph" && curSels.length === 1 &&
            ...
            if (proceedingBlock && directionUP) {
                activeEditor.setSelection({ line: proceedingBlock.position.start.line, ch: 0 },
