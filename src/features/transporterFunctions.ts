...
export async function copyOrPushLineOrSelectionToNewLocation(
  ...
  if (targetFileLineNumber === -1) {
    ...
    if (f.details.length > 0 && f.details[0].type === 'yaml') targetFileLineNumber = f.details[0].lineEnd;
  }
  targetFileContentsArray.splice(Number(targetFileLineNumber) + 1, 0, { display: newText, info: '' });
  let newContents = '';
  for (const line of targetFileContentsArray) newContents += line.display + '\n';
...
export async function copyOrPushLineOrSelectionToNewLocationWithFileLineSuggester(
  ...
  await displayFileLineSuggester(
    ...
    async (targetFileName, fileContentsArray, lineNumber, endLineNumber, evtFileSelected, evtFirstLine) => {
      ...
      if (
        (evtFileSelected && (evtFileSelected.ctrlKey || evtFileSelected.metaKey)) ||
...
export async function copyOrPushLineOrSelectionToNewLocationUsingCurrentCursorLocationAndBoomark(
  ...
  if (bookmarkInfo.errorNumber === 1) new Notice('Location in the bookmark does not exist.');
  else if (bookmarkInfo.errorNumber === 2) new Notice('File as defined in the bookmark does not exist.');
  else {
    const activeEditor = getActiveView(plugin).editor;
    const currentLine = activeEditor.getCursor().line;
...
export async function copyCurrentFileNameAsLinkToNewLocation(plugin: TextTransporterPlugin, copyToCliboard: boolean): Promise<void> {
  const fileLink = '[[' + getUniqueLinkPath(getActiveView(plugin).file.path) + ']]';
  if (copyToCliboard) {
    navigator.clipboard.writeText(fileLink).then((text) => text);
    new Notice(`${fileLink}\n\n Copied to the clipboard.`);
  } else copyOrPushLineOrSelectionToNewLocationWithFileLineSuggester(plugin, true, fileLink);
}

//copy a block reference of the current line to another file
export async function pushBlockReferenceToAnotherFile(plugin: TextTransporterPlugin): Promise<void> {
  await displayFileLineSuggester(
    plugin,
    false,
    true,
    ...
    async (targetFileName, fileContentsArray, startLine, endLineNumber, evtFileSelected, evtFirstLine) => {
      if (startLine === -1) {
        //go to top of file, but test for YAML
        const f = new FileCacheAnalyzer(plugin, targetFileName);
        if (f.details.length > 0 && f.details[0].type === 'yaml') startLine = f.details[0].lineEnd;
      }
      ...
      if (results.length > 0) {
        ...
        let newContents = '';
        for (const line of fileContentsArray) newContents += line.display + '\n';
        newContents = newContents.substring(0, newContents.length - 1);
...
export async function copyOrPulLineOrSelectionFromAnotherLocation(plugin: TextTransporterPlugin, copySelection: boolean): Promise<void> {
  await displayFileLineSuggester(
    plugin,
    true,
    false,
    ...
    async (targetFileName, fileContentsArray, startLine, endLine, evtFileSelected, evtFirstLine, evetLastLine) => {
      const ctrlKey =
        (evtFileSelected && evtFileSelected.ctrlKey) || (evtFirstLine && evtFirstLine.ctrlKey) || (evetLastLine && evetLastLine.ctrlKey);
      startLine = startLine === -1 ? (startLine = 0) : startLine;
      endLine = endLine === -1 ? (endLine = 0) : endLine;
      let stringToInsertIntoSelection = '';
      for (const element of fileContentsArray.slice(startLine, endLine + 1)) stringToInsertIntoSelection += element.display + '\n';
      ...
      } else if (ctrlKey) await openFileInObsidian(plugin, targetFileName, startLine, endLine - startLine);
    }
  );
}

// pull a block reference from another file and insert into the current location
export async function pullBlockReferenceFromAnotherFile(plugin: TextTransporterPlugin): Promise<void> {
  await displayFileLineSuggester(
    plugin,
    true,
    false,
    ...
    async (targetFileName, fileContentsArray, startLine, endLine, evtFileSelected, evtFirstLine, evetLastLine) => {
      ...
      if (blockRefs.length > 0) {
        ...
        for (const ref of blockRefs) blockRefTextToInsert += `![[${targetFileName}${ref}]]\n`;
...
export function testIfCursorIsOnALink(plugin: TextTransporterPlugin): LinkCache {
  const activeView = getActiveView(plugin);
  const activeEditor = activeView.editor;
  const currentLine = activeEditor.getCursor().line;
  ...
  if (cache.links || cache.embeds || cache.headings) {
    ...
    if (cache.links)
      linkInfo = cache.links.find(
        (l: LinkCache) => l.position.start.line === currentLine && ch >= l.position.start.col && ch <= l.position.end.col
      );
