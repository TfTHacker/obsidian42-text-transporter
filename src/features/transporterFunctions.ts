import { type LinkCache, Notice, type TFile, getLinkpath } from "obsidian";
import type TextTransporterPlugin from "../main";
import type { SuggesterItem } from "../ui/GenericFuzzySuggester2";
...
import {
	...
	getUniqueLinkPath,
	openFileInObsidian,
	parseBookmarkForItsElements,
} from "../utils/fileNavigatior";
...
export async function addBlockRefsToSelection(
	plugin: TextTransporterPlugin,
	copyToClipbard: boolean,
	copyAsAlias = false,
	aliasText = "*",
	...
	for (const sel of curSels) {
		const startLine =
			sel.anchor.line > sel.head.line ? sel.head.line : sel.anchor.line;
		const endLine =
			sel.anchor.line > sel.head.line ? sel.anchor.line : sel.head.line;
		for (
			let selectedLineInEditor = startLine;
			selectedLineInEditor <= endLine;
			selectedLineInEditor++
		) {
			for (
				let sectionCounter = 0;
				sectionCounter < f.details.length;
				sectionCounter++
			) {
				const section = f.details[sectionCounter];
				if (
					selectedLineInEditor >= section.position.start.line &&
					...
					if (
						...
						activeEditor.replaceRange(
							` ^${newId}`,
							{
								line: Number(section.position.end.line),
					...
					if (section.type === "heading") {
						blockRefs.push(
							`#${cleanupHeaderNameForBlockReference(section.headingText)}`,
						);
						selectedLineInEditor = section.position.end.line;
						break;
					}
	...
	if (copyToClipbard && blockRefs.length > 0) {
		...
		blockRefs.forEach(
			...
			(b) => (block += `${blockPrefix}[[${uniqueLinkPath}${b}${aliasText}]]\n`),
		);
...
export async function copyOrPushLineOrSelectionToNewLocation(
	plugin: TextTransporterPlugin,
	copySelection: boolean,
	newText: string,
	targetFileName: string,
	targetFileLineNumber: number,
	targetFileContentsArray: Array<SuggesterItem>,
): Promise<void> {
	if (targetFileLineNumber === -1) {
		//go to top of file, but test for YAML
	...
	targetFileContentsArray.splice(Number(targetFileLineNumber) + 1, 0, {
		display: newText,
		info: "",
	});
	let newContents = "";
	...
	if (copySelection === false) {
		//this  is  a move, so delete the selection
		...
		if (
			textSelection === "" ||
			activeEditor.getLine(currentLine).length === textSelection.length
		)
			activeEditor.replaceRange(
				"",
		...
		else activeEditor.replaceSelection(""); //replace whatever is the  selection
	}
}
...
export async function copyOrPushLineOrSelectionToNewLocationWithFileLineSuggester(
	plugin: TextTransporterPlugin,
	copySelection: boolean,
	defaultSelectionText = "",
): Promise<void> {
	const activeEditor =
		defaultSelectionText === "" ? getActiveView(plugin).editor : null;
	let selectedText =
		defaultSelectionText === ""
			? activeEditor.getSelection()
			: defaultSelectionText;
	if (selectedText === "")
		selectedText = activeEditor.getLine(activeEditor.getCursor().line); //get text from current line
	await displayFileLineSuggester(
		...
		async (
			...
			await copyOrPushLineOrSelectionToNewLocation(
				plugin,
				copySelection,
				selectedText,
				targetFileName,
			...
			if (
				(evtFileSelected &&
					(evtFileSelected.ctrlKey || evtFileSelected.metaKey)) ||
				(evtFirstLine && (evtFirstLine.ctrlKey || evtFirstLine.metaKey))
			) {
...
export async function copyOrPushLineOrSelectionToNewLocationUsingCurrentCursorLocationAndBoomark(
	plugin: TextTransporterPlugin,
	copySelection: boolean,
	bookmarkText: string,
	evt?: MouseEvent | KeyboardEvent,
): Promise<void> {
	const bookmarkInfo = await parseBookmarkForItsElements(
		plugin,
		bookmarkText,
		false,
	);
...
export async function copyCurrentFileNameAsLinkToNewLocation(
	plugin: TextTransporterPlugin,
	copyToCliboard: boolean,
): Promise<void> {
...
export async function pushBlockReferenceToAnotherFile(
	plugin: TextTransporterPlugin,
): Promise<void> {
	await displayFileLineSuggester(
		plugin,
		...
		async (
			...
			if (results.length > 0) {
				for (const ref of results) blockRefs += `![[${fileName}${ref}]]\n`;
				blockRefs = blockRefs.substring(0, blockRefs.length - 1);
				fileContentsArray.splice(Number(startLine) + 1, 0, {
					display: blockRefs,
					info: "",
				});
				let newContents = "";
...
export async function copyOrPulLineOrSelectionFromAnotherLocation(
	plugin: TextTransporterPlugin,
	copySelection: boolean,
): Promise<void> {
	await displayFileLineSuggester(
		...
		async (
			...
			const ctrlKey =
				evtFileSelected?.ctrlKey ||
				evtFirstLine?.ctrlKey ||
				evetLastLine?.ctrlKey;
			// biome-ignore lint/suspicious/noAssignInExpressions: <explanation>
			...
			stringToInsertIntoSelection = stringToInsertIntoSelection.substring(
				0,
				stringToInsertIntoSelection.length - 1,
			);
			getActiveView(plugin).editor.replaceSelection(
				stringToInsertIntoSelection,
			);
			if (copySelection === false) {
...
export async function pullBlockReferenceFromAnotherFile(
	plugin: TextTransporterPlugin,
): Promise<void> {
	await displayFileLineSuggester(
		plugin,
		...
		async (
			...
			const f = new FileCacheAnalyzer(plugin, targetFileName);
			const fileContents = (
				await plugin.app.vault.adapter.read(targetFileName)
			).split("\n");
					...
					if (
						...
						if (
							...
							fileContents.splice(
								...
								1,
								`${fileContents[section.position.end.line]} ^${newId}`,
							);
							blockRefs.push(`#^${newId}`);
							fileChanged = true;
							lineNumber = section.position.end.line;
						...
						if (section.type === "heading") {
							const heading = cleanupHeaderNameForBlockReference(
								section.headingText,
							);
			...
			if (fileChanged === true) {
				let newContents = "";
				for (const line of fileContents) newContents += `${line}\n`;
				newContents = newContents.substring(0, newContents.length - 1);
				await plugin.app.vault.adapter.write(targetFileName, newContents);
			}
			...
			if (blockRefs.length > 0) {
				...
				blockRefTextToInsert = blockRefTextToInsert.substring(
					0,
					blockRefTextToInsert.length - 1,
				);
...
export function testIfCursorIsOnALink(
	plugin: TextTransporterPlugin,
): LinkCache {
	const activeView = getActiveView(plugin);
	const activeEditor = activeView.editor;
	...
	if (cache.links || cache.embeds || cache.headings) {
		...
		if (cache.links)
			linkInfo = cache.links.find(
				(l: LinkCache) =>
					l.position.start.line === currentLine &&
					ch >= l.position.start.col &&
					ch <= l.position.end.col,
			);
...
export async function copyBlockReferenceToCurrentCusorLocation(
	plugin: TextTransporterPlugin,
	linkInfo: LinkCache,
	leaveAliasToFile: boolean,
): Promise<void> {
	const file: TFile = plugin.app.metadataCache.getFirstLinkpathDest(
		getLinkpath(linkInfo.link),
		"/",
	);
	...
	if (cache.details && linkInfo.link.includes("^")) {
		...
		const pos = cache.details.find(
			(b: CacheDetails) => b.blockId === blockRefId,
		).position;
