...
export class SilentFileAndTagSuggester extends TextInputSuggest<string> {
	...
	getSuggestions(inputStr: string): string[] {
		...
		if (tagMatch) {
			...
			suggestions = this.tags.filter((tag) =>
		...
		if (fileLinkMatch) {
			...
			suggestions = this.files
				.filter((file) =>
					file.path.toLowerCase().contains(fileNameInput.toLowerCase()),
				)
				.map((file) => file.path);
			suggestions.push(
				...this.unresolvedLinkNames.filter((name) =>
					name.toLowerCase().contains(fileNameInput.toLowerCase()),
				),
			);
		}
	...
	selectSuggestion(item: string): void {
		...
		if (this.lastInputType === TagOrFile.File) {
			const linkFile: TAbstractFile =
				this.app.vault.getAbstractFileByPath(item);
