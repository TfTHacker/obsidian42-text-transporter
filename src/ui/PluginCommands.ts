...
export default class PluginCommands {
  ...
  commands = [
    ...
    },
    {
      ...
      command: async (): Promise<void> => {
        ...
        if (linkInfo) await transporter.copyBlockReferenceToCurrentCusorLocation(this.plugin, linkInfo, false);
    ...
    },
    {
      ...
      command: async (): Promise<void> => {
        const linkInfo = transporter.testIfCursorIsOnALink(this.plugin);
        if (linkInfo) await transporter.copyBlockReferenceToCurrentCusorLocation(this.plugin, linkInfo, true);
        else new Notice('No link selected in editor.');
      }
    },
    {
  ...
  async masterControlProgram(plugin: TextTransporterPlugin): Promise<void> {
    ...
    for (const cmd of this.commands) {
      ...
      if (cmd.group === 'replace' && activeView === ViewType.source && transporter.testIfCursorIsOnALink(this.plugin)) addCommand = true;
      else if (cmd.group !== 'replace' && (cmd.editModeOnly === false || (editMode && cmd.editModeOnly))) addCommand = true;
      else if ((cmd.shortcut === 'SLF' || cmd.shortcut === 'SLC') && activeView != ViewType.none) {
        //send command. show file exists
        addCommand = true;
      }
      if (addCommand) cpCommands.push({ display: `${cmd.caption} (${cmd.shortcut})`, info: cmd.command });
    }
      ...
      for (const bookmark of plugin.settings.bookmarks.split('\n')) {
        if (bookmark.substr(0, 1) === '*') {
          cpCommands.push({
            ...
            info: async (e) => {
              await transporter.copyOrPushLineOrSelectionToNewLocationUsingCurrentCursorLocationAndBoomark(plugin, true, bookmark, e);
            }
          });
          cpCommands.push({
            display: `   Push: ${bookmark}`,
            info: async (e) => {
              await transporter.copyOrPushLineOrSelectionToNewLocationUsingCurrentCursorLocationAndBoomark(plugin, false, bookmark, e);
            }
          });
        }
      }
    ...
    this.plugin.registerEvent(
      this.plugin.app.workspace.on('editor-menu', (menu) => {
        ...
        for (const value of this.commands) {
          let addCommand = false;
          if (value.cmItemEnabled === true && value.group !== 'replace') addCommand = true;
